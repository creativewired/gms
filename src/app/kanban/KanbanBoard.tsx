...
export default function KanbanBoard({
  initialJobs, branches, activeBranchId,
}: {
  initialJobs: Job[];
  branches: Branch[];
  ...
  const handleDrop = async (e: React.DragEvent, newStatus: string) => {
    ...
    if (!job || job.status === newStatus) { setDragging(null); return; }
  ...
  const handleMobileStatusChange = async (job: Job, newStatus: string) => {
  ...
  const JobCard = ({ job, col }: { job: Job; col: typeof COLUMNS[0] }) => (
    <div
      draggable
      ...
      style={{
        ...
        boxShadow: dragging === job.id ? "0 8px 24px rgba(0,0,0,0.12)" : "0 1px 4px rgba(0,0,0,0.04)",
      }}
    >
      <div className="p-3">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-mono text-slate-400">#{String(job.id).padStart(4, "0")}</span>
          {updating === job.id ? (
            ...
            <Link href={`/jobs/${job.id}`} onClick={e => e.stopPropagation()}
              className="text-xs text-slate-300 hover:text-slate-600 transition-colors">→</Link>
          )}
        </div>
        {job.vehicles && (
          <div className="flex items-center gap-1.5 mb-2">
            <div className="w-5 h-5 rounded-md flex items-center justify-center shrink-0" style={{ background: col.bg }}>
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold font-mono text-slate-800 truncate">{job.vehicles.plate_number}</p>
              {(job.vehicles.make || job.vehicles.model) && (
                <p className="text-xs text-slate-400 truncate">{job.vehicles.make} {job.vehicles.model}</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          {job.mechanic_name && (
            <span className="text-xs text-slate-400 truncate max-w-[80px]">{job.mechanic_name}</span>
          )}
        </div>
        ...
        <div className="md:hidden mt-2 pt-2" style={{ borderTop: "1px solid rgba(0,0,0,0.05)" }}>
          <select
            value={job.status}
            onChange={e => handleMobileStatusChange(job, e.target.value)}
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {COLUMNS.map(col => {
          ...
          return (
            <div key={col.key} className="stat-card" style={{ borderTop: `3px solid ${col.border}` }}>
              <div className="flex items-center gap-2 mb-1">
                <div className="w-2 h-2 rounded-full" style={{ background: col.dot }} />
                <p className="label text-xs">{col.label}</p>
              </div>
              <p style={{ fontSize: "1.4rem", fontWeight: 700, color: col.color, letterSpacing: "-0.02em", lineHeight: 1 }}>
                {colJobs.length}
              </p>
        ...
        {COLUMNS.map(col => {
          ...
          return (
            <button key={col.key} onClick={() => setActiveCol(col.key)}
              ...
              style={{
                background: activeCol === col.key ? col.bg : "#f2f2f7",
                color: activeCol === col.key ? col.color : "#6b7280",
                border: activeCol === col.key ? `1px solid ${col.border}` : "1px solid transparent",
              }}>
            ...
            <div key={col.key} className="space-y-2">
              {colJobs.length === 0 ? (
                <div className="card p-10 text-center">
                  <p className="text-slate-300 text-sm">No {col.label.toLowerCase()} jobs</p>
                </div>
              ) : colJobs.map(job => (
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                {colJobs.map(job => <JobCard key={job.id} job={job} col={col} />)}
              </div>
            </div>
          );
        })}
