...
const insightConfig = {
  positive:    { bg: "#f0fdf4", border: "#bbf7d0", icon: "#22c55e", badge: "bg-emerald-50 text-emerald-600 border-emerald-100", label: "Positive" },
  warning:     { bg: "#fffbeb", border: "#fde68a", icon: "#f59e0b", badge: "bg-amber-50 text-amber-600 border-amber-100",   label: "Warning" },
  opportunity: { bg: "#eff6ff", border: "#bfdbfe", icon: "#3b82f6", badge: "bg-blue-50 text-blue-600 border-blue-100",      label: "Opportunity" },
  alert:       { bg: "#fff1f0", border: "#fecaca", icon: "#ef4444", badge: "bg-red-50 text-red-500 border-red-100",          label: "Alert" },
};
...
export default function AIInsightsClient({ branchId, branches }: { branchId: number | null; branches: Branch[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl p-5 sm:p-8"
        ...
        <div style={{ position: "absolute", top: "-60px", right: "-40px", width: "250px", height: "250px", borderRadius: "50%", background: "rgba(167,139,250,0.1)", filter: "blur(40px)" }} />
        <div style={{ position: "absolute", bottom: "-40px", left: "20%", width: "180px", height: "180px", borderRadius: "50%", background: "rgba(99,102,241,0.15)", filter: "blur(30px)" }} />
        ...
        <div className="relative z-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <p style={{ color: "rgba(255,255,255,0.45)", fontSize: "0.8rem", marginTop: "0.4rem", maxWidth: "380px", lineHeight: 1.5 }}>
                AI analyzes your jobs, revenue, services, mechanics, and appointments to surface actionable insights and revenue forecasts.
              </p>
            </div>

            {/* Branch selector */}
            <div className="shrink-0">
              <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "0.65rem", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "0.4rem" }}>
                Branch
              </p>
              <select
                ...
                style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", color: "white", minWidth: "140px" }}>
                <option value="" style={{ color: "#1c1c1e" }}>All Branches</option>
                {branches.map(b => <option key={b.id} value={b.id} style={{ color: "#1c1c1e" }}>{b.name}</option>)}
              </select>
            </div>
          </div>

          <button onClick={handleAnalyze} disabled={loading}
            className="mt-5 flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all w-full sm:w-auto justify-center sm:justify-start"
            style={{
        ...
        <div className="space-y-5">
          ...
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Executive Summary</p>
            </div>
            <p className="text-slate-700 text-sm leading-relaxed">{result.summary}</p>
          </div>

          {/* Key Metrics: 2×2 on mobile, 4 cols on sm+ */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Busiest Day",    value: result.busiest_day,  icon: "📈", color: "#15803d", border: "#bbf7d0" },
              { label: "Slowest Day",    value: result.slowest_day,  icon: "📉", color: "#dc2626", border: "#fecaca" },
              { label: "Top Service",    value: result.top_service,  icon: "⭐", color: "#b45309", border: "#fde68a" },
              {
                label: "Revenue Trend",
              ...
              <div key={m.label} className="stat-card" style={{ borderTop: `3px solid ${m.border}` }}>
                <p className="label text-xs">{m.label}</p>
          ...
          <div className="rounded-2xl p-5 sm:p-6"
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div>
                ...
                <p style={{ fontSize: "1.75rem", fontWeight: 800, color: "#4c1d95", letterSpacing: "-0.03em", lineHeight: 1 }}>
                  {result.forecast.next_month_estimate}
                </p>
                <p className="text-sm text-slate-500 mt-2 max-w-sm">{result.forecast.reasoning}</p>
              </div>
              <span className={`self-start px-3 py-1 rounded-full text-xs font-semibold border shrink-0 ${
                result.forecast.confidence === "high"   ? "bg-emerald-50 text-emerald-600 border-emerald-100" :
                result.forecast.confidence === "medium" ? "bg-amber-50 text-amber-600 border-amber-100" :
          ...
          {/* Insights Grid */}
          <div>
            <p className="section-title mb-3">Insights</p>
                  ...
                  <div key={i} className="card p-4 sm:p-5"
                    ...
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs font-bold" style={{ color: config.icon }}>{insight.metric}</p>
                      </div>
                    </div>
                    <p className="text-xs text-slate-500 leading-relaxed mb-3">{insight.description}</p>
                    <div className="flex items-start gap-2 rounded-xl p-3" style={{ background: config.bg }}>
                      <svg className="w-3.5 h-3.5 shrink-0 mt-0.5" fill="none" stroke={config.icon}
                        strokeWidth={2.5} viewBox="0 0 24 24">
                        <polyline points="9 18 15 12 9 6" />
                      </svg>
                      <p className="text-xs font-medium" style={{ color: config.icon }}>{insight.action}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {!result && !loading && !error && (
        <div className="card p-10 sm:p-16 text-center">
          ...
          <p className="text-slate-700 font-semibold text-sm sm:text-base">Ready to analyze</p>
